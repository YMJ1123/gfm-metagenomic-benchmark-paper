...
\begin{table}[h]
\centering
\caption{$k$-mer classifiers on the same 50M-read reference and test pool.
The 13-mer network is not exact-match lookup; composition (naive Bayes) already
reaches 74.9\%, and the network adds a further $+13$ pp.}
\small
\begin{tabularx}{\linewidth}{@{}Xc@{}}
\toprule
Method & Genus Top-1 \\
\midrule
Unique-key lookup (Kraken-style, $k{=}13$) & 0.72\% \\
Dominant-genus majority vote & 35.3\% \\
Multinomial 13-mer naive Bayes & 74.9\% \\
MT 13-mer neural (50M) & 87.5\% \\
NT-v2 6-mer (498M pre-trained) & 67\% \\
\bottomrule
\end{tabularx}
\end{table}

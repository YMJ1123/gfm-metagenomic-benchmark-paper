...
sets the ceiling: a 13-mer model trained from scratch reaches 87.4\% genus Top-1
versus 67.1\% for a pre-trained 6-mer model on identical 50M-read data, and it
keeps scaling to 250M ($87.4\%\!\rightarrow\!98.7\%$) while the 6-mer model
saturates ($+0.22$ pp) and cannot even \emph{fit} its training set beyond
$\sim$66\%---a representational, not optimization, ceiling. With tokenization
fixed, pre-training adds $+13.2$ pp. At the sample level a 67\%-accurate model

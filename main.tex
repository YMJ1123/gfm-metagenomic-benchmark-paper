...
(87.5\% to 98.7\%); the 6-mer model fails even to \emph{fit} its training set
...
abundance. On a real mock community, however, all methods (neural and
Kraken\,2$+$Bracken) fall to $r\approx0.4$--$0.6$ with no clear winner, exposing
a large closed-set-to-real gap that is the binding open problem.
...
classifier reaches Pearson $r=0.99$ for genus abundance, but low read accuracy
still cripples low-abundance detection.

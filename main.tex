...
(87.5\% to 98.7\%). With tokenization fixed at 6-mer, pre-training contributes
$+13.2$ percentage points. At the sample level, a 67\%-accurate read classifier
attains Pearson $r=0.99$ for genus abundance---exceeding Kraken\,2
($r=0.82$), whose $\sim$30\% abstention uniformly deflates predicted
abundances---yet Kraken\,2 remains far stronger for low-abundance detection.
...
\begin{itemize}
\item Under a fixed backbone, training-data volume dominates accuracy only up to
a tokenizer-imposed ceiling: a non-overlapping 6-mer model saturates beyond 50M
reads ($+0.22$ percentage points from 50M to 250M).

...
does not capture all real sequencing artefacts. Our preliminary probe on a real
mock community (ZymoBIOMICS D6331, Section~\ref{sec:mock}), now with a matched
Kraken\,2$+$Bracken baseline, shows all methods dropping to $r\approx0.42$--$0.58$
with no clear winner and Kraken\,2 abstaining on 56\% of reads---a substantial
closed-set-to-real gap. That single small community (11 in-set genera, 135\,bp
reads) is not a definitive benchmark; a fuller evaluation across multiple
communities, matched read lengths, and additional baselines (e.g.\ Centrifuge)

...
\begin{tikzpicture}[
  ...
  % middle + right as a 3x2 matrix so rows share centres
  \matrix[
      column sep=12mm,
      row sep=3.5mm,
      nodes={anchor=center}
    ] (M) {
    \node[axis] (a1) {\textbf{Data scale}\\500K\,$\to$\,250M}; &
    \node[ev]   (e0) {\textbf{Sample-level}\\abundance ($r$, Bray--Curtis)}; \\
    \node[axis] (a2) {\textbf{Pre-training}\\pretrained vs scratch}; &
    \node[ev]   (e1) {\textbf{Read-level}\\Top-1 accuracy}; \\
    \node[axis] (a3) {\textbf{Tokenization}\\6/12/13-mer, overlap}; &
    \node[ev]   (e2) {\textbf{Detection}\\ROC, sens.\ @ 95\% spec.}; \\
  };

  % left column, vertically centred on the middle row
  ...
  % fork: stem from reads → vertical spine → three axes
  ...
  % axis → evaluation (same row centres)
  \draw[ar] (a1) -- (e0);
  \draw[ar] (a2) -- (e1);
  \draw[ar] (a3) -- (e2);

  % Kraken under Detection, clear spacing

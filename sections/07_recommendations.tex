...
For in-database samples, Kraken\,2$+$Bracken is excellent ($r=0.997$) and has no
neural competitor; a 67\%-accurate neural model is comparable ($r=0.99$) and
useful when the reference is incomplete, but do not expect it to beat a
Bracken-corrected pipeline. Compare against raw counts \emph{and} Bracken. \\[2pt]
...
\item \textbf{Low-abundance presence/absence?} Use an exact-$k$-mer pipeline
(Kraken\,2/Bracken, Centrifuge); current 6-mer neural detectors are weak.
...
\item \textbf{Community abundance (in-database)?} Use Kraken\,2$+$Bracken
($r=0.997$); a neural model is comparable but not better. Reach for the neural
model mainly when the reference is incomplete.

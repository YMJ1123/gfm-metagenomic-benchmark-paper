...
than 13 accuracy points that no amount of extra 6-mer data recovers. This benefit
is not specific to one backbone: across pre-trained GFMs fine-tuned identically at
5M reads, NT-v2 was best (63.1\% genus Top-1) ahead of DNABERT (61.8\%) and
DNABERT-2 (58.9\%), yet all three remained bounded well below the
overlapping-13-mer ceiling---their (sub-)6-mer-scale tokenizers, not the choice of
backbone, are the limiting factor (Supplementary).
...
representations plateau at 45--67\% regardless of pre-training or data. NT-v2
values use reverse-complement test-time augmentation (RC-TTA); MT values are
forward-only (RC-TTA adds $\sim$0.7 pp), so the tokenization gap is if anything
...
($87.4\%\!\rightarrow\!98.7\%$) while the 6-mer NT-v2 is flat ($+0.22$ pp;
...
further $+13$ pp over naive Bayes (87.4\%), capturing co-occurrence structure
...
\caption{\textbf{Tokenization dominates the model.} Genus Top-1 accuracy with
the classifier held fixed---a non-neural multinomial $k$-mer naive Bayes---or
as a neural network, for 6-mer versus overlapping 13-mer tokenization (same
50M-read reference and test pool). With the method fixed, moving to 13-mer nearly
triples accuracy ($25.9\%\!\rightarrow\!74.9\%$), isolating $k$-mer length as the
decisive variable; the neural gain is larger on the weaker tokenizer ($+41$ pp on
6-mer versus $+13$ pp on 13-mer). The non-neural 13-mer naive Bayes (74.9\%)
already exceeds the 498M-parameter pre-trained NT-v2 on 6-mer (67\%). Exact-match
unique-key lookup fails (0.7\%), confirming this is learned composition, not
lookup.}
\label{fig:kmer}
\end{figure}

This analysis also isolates $k$-mer length as the decisive variable. Holding the
classifier fixed and applying the same multinomial naive Bayes to 6-mer
composition (NT-v2's tokenization) yields only 25.9\%: moving from 6-mer to
13-mer alone nearly triples accuracy ($25.9\%\!\rightarrow\!74.9\%$) with no
change to the model. Conversely, the neural network contributes \emph{more} when
the tokenizer is weaker---it adds $+41$ pp on 6-mer (25.9\% naive Bayes to 67.0\%
for the 498M-parameter NT-v2) but only $+13$ pp on 13-mer (74.9\% to 87.4\%)---so
a poorer input representation forces the model to work harder to recover the same
signal. Most strikingly, the non-neural 13-mer naive Bayes (74.9\%) still
\emph{beats} the pre-trained 498M-parameter NT-v2 on 6-mer (67\%): no amount of
capacity or pre-training on 6-mer catches trivial statistics on 13-mer.
Representation ($k$-mer length) is more decisive than model capacity or
pre-training. We are explicit about the accompanying caveat: because the
reference genomes are nearly
